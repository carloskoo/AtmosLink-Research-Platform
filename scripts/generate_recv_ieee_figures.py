...
PHYSICAL_COLOR = "#C62828"
CALIBRATED_COLOR = "#1565C0"
REFERENCE_COLOR = "#455A64"
...
def plot_observed_vs_predicted(
    ...
    axis.plot(
        ...
        linewidth=1.4,
        color=PHYSICAL_COLOR
        if "physical" in predicted_column
        else CALIBRATED_COLOR,
...
def plot_residual_boxplot(
    ...
    annotation_box(
        axis,
        (
            f"Physical MAE = {physical_mae:.3f} dB\n"
            f"Calibrated MAE = {calibrated_mae:.3f} dB\n"
...
def plot_overall_validation_summary(
    pooled_metrics: pd.DataFrame,
    output_directory: Path,
    dpi: int,
) -> None:
    """
    Generate a four-panel summary of pooled RECV performance.
    """
    ...
    figure.suptitle(
        (
            "Overall pooled RECV validation summary "
            f"(MAE reduction = {mae_reduction:.2f}%, "
    ...
    save_all_formats(
        figure,
        output_directory,
        "Fig_11_overall_recv_summary_ieee",
        dpi,
    )
...
def main() -> None:
    ...
    plot_overall_validation_summary(
        pooled_metrics,
        output_directory,
        arguments.dpi,
    )
